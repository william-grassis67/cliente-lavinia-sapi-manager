function Notificacao() {
    const notificacoesSuportadas = typeof Notification !== "undefined";

    async function solicitarPermissao() {
        if (!notificacoesSuportadas) {
            console.warn("[Notificacao] Notificações não são suportadas neste navegador.");
            window.SapiToast?.info("Seu navegador não suporta notificações.");
            return;
        }
        ...
        try {
            ...
            console.error("[Notificacao] Erro ao solicitar permissão:", erro);
        }
    }
    ...
    if (btnConfirmNotification) {
        btnConfirmNotification.addEventListener("click", solicitarPermissao);
    }

    function verificarVencimento() {
        if (!notificacoesSuportadas || Notification.permission !== "granted") {
            return;
        }
